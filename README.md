...
## Preview Jpg compression findings. 

- 8×8 → mean=0.616, std=0.767- Low variance at the true JPEG block size. Canon DIGIC IV hardware always compresses at this level. 
- 16×16 → mean=0.925, std=1.668 - Slightly higher, but still close to natural scaling. 
- 32×32 → mean=1.466, std=3.482 - Growing variance, expected as we aggregate over larger blocks. 
- 128×128 → mean=4.421, std=9.977 - Disproportionately high. The variance here is 7× greater than at 8×8, which indicates strong periodic structure at 128×128

  For each 8×8 block in the preview image, the script calculated the variance of pixel intensities inside that block.

Then it averaged those variances across the whole image (the mean) and also measured how much those variances themselves fluctuated across different blocks (the std).

So:

Mean = 0.616 → on average, each 8×8 block has very little pixel fluctuation. That’s low compared to natural image structure, showing weak 8×8 periodic signal.

Std = 0.767 → the spread of those block variances is also low, meaning most 8×8 blocks behave similarly.

In a real Canon preview, you’d expect the 8×8 level to show a stronger “signature” (higher mean variance compared to larger blocks), because DIGIC IV encodes at 8×8. Here, the 128×128 level shows the dominant signal instead.

Low variance at 8×8 in a sky region is expected. Smooth areas like sky naturally give low per-block variance. But this should also hold at 128×128 if the encoder is Canon’s DIGIC IV, because variance should scale smoothly with block size. What we observed is not just “low sky variance.”

```

At 8×8: mean variance = 0.616.

At 128×128: mean variance = 4.421.

That’s a ~7× jump, which doesn’t come from the sky — it comes from periodic artifacts at that block size.

Observed values from your CR2 preview

8×8 → mean=0.616, std=0.767 : Each 8×8 block has very little intensity fluctuation. Standard deviation is also small, so block-to-block behavior is uniform. With clouds present, this should normally be higher — weak 8×8 signal is not consistent with Canon’s native encoder.

16×16 → mean=0.925, std=1.668: Variance increases slightly as expected when combining four 8×8 blocks. Nothing anomalous yet — this growth is natural scaling.

32×32 → mean=1.466, std=3.482: Variance continues to grow. Still within expectation: larger areas capture more texture variation from clouds.

128×128 → mean=4.421, std=9.977: Sharp, disproportionate increase. The jump is ~7× compared to 8×8. This indicates a repeating structure aligned with 128×128 boundaries.

Canon DIGIC IV cannot produce such periodicity: its JPEG encoder is hardwired for 8×8 DCT.

Standard deviation values : Std values (0.767 → 9.977) show how much variance fluctuates across different regions.

The explosive growth at 128×128 means the periodic structure is global across the frame, not just local features (clouds).

### **This is exactly the footprint of a recompressed image with large-block artifacts.**

##Final interpretation

In a genuine Canon EOS 5D Mark II CR2: 8×8 variance would be the strongest signal, since compression occurs at 8×8. Larger block sizes would show smooth, proportional growth only.

In Jonas CR2:

8×8 signal is weak despite clouds, and i created using a synthetic process outside the Canon Camera. 

128×128 dominates, proving the embedded JPEG was generated by a non-Canon process.

```
This is the internal contradiction: the claimed Canon CR2 preview exhibits block periodicity Canon’s DIGIC IV encoder cannot produce.

Canon baseline (what matters): As per Canon's response: Their EOS Mark II Hardware encoder only uses 8×8 DCT. So any dominant periodicity at 128×128 is impossible regardless of scene content. Sky or no sky, DIGIC IV does not magically switch to 128×128.
Therefore the Image is not native to the "Canon EOS 5D Mark II" or variant Camera. 
